...
from ..kernels import SquaredExponential, Matern32, Hybrid
from ..layers import Gaussian, StudentT
...
from ..layers import GPLayer, LikelihoodLayer, DistGPLayer
from ..layers import LikelihoodLayer
from ..models import DistDeepGP
from ..mean_functions import Zero, Identity
...
def _construct_euclidean_kernel(input_dim: int, is_last_layer: bool) -> SquaredExponential:
    ...
    return Matern32(lengthscales=lengthscales, variance=variance)
...
def _construct_hybrid_kernel(input_dim: int, is_last_layer: bool) -> Hybrid:
    ...
    return Hybrid(baseline_kernel = 'matern32', lengthscales=lengthscales, variance=variance)


def build_constant_input_dim_dist_deep_gp_matern_studentt(X: np.ndarray, num_layers: int, config: Config, dim_output : int) -> DistDeepGP:
    r"""
    ...
    for i_layer in range(1, num_layers):
        ...
        kernel = construct_basic_kernel(
            kernels=_construct_hybrid_kernel(D_in, is_last_layer),
            output_dim=D_out,
            share_hyperparams=True,
        )
    ...
    likelihood = StudentT(scale=1e-2)

    return DistDeepGP(gp_layers, LikelihoodLayer(likelihood))
