...
class IndependentHeteroskedasticOrthogonalPosteriorMultiOutput(IndependentOrthogonalPosterior):

    # NOTE -- this is just an extra class to deal with the Heteroskedastic scenario
    ...
    def _conditional_fused(
        ...
        if isinstance(self.inducing_variable_u, SharedIndependentInducingVariables) and isinstance(
            ...
            fmean, fvar = base_heteroskedastic_orthogonal_conditional(
                Kmn, Kmm, Knn, Cmn, Cmm, Cnn,
                self.q_mu_u, self.q_mu_v, full_cov=full_cov, q_sqrt_u=self.q_sqrt_u, q_sqrt_v=self.q_sqrt_v , white=self.whiten
            )  # [N, P],  [P, N, N] or [N, P]
