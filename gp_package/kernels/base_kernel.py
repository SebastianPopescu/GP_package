...
class DistributionalKernel(Module, metaclass=abc.ABCMeta):
    ...
    def __call__(
        ...
        *,
        full_cov: bool = True,
        presliced: bool = False) -> tf.Tensor:
        ...
        if not full_cov:
            assert X2 is None
            assert X2_moments is None

            return self.K_diag(X)

        else:
            
            return self.K(X, X_moments, X2, X2_moments)
...
class ReducingCombination(DistributionalCombination):
    def __call__(
        ...
        *,
        ...
        return self._reduce(
            [k(X, X_moments, X2, X2_moments, full_cov=full_cov) for k in self.kernels]
        )
    ...
    def K(self, X: TensorType, 
        ...
        return self._reduce([k.K(X, X_moments, X2, X2_moments) for k in self.kernels])
