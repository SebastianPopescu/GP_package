...
class BayesianKernel(Module, metaclass=abc.ABCMeta):
    """
    ...
    def on_separate_dims(self, other: "BayesianKernel") -> bool:
        """
    ...
    def __call__(
        ...
        *,
        ...
        if not full_cov:
            assert X2 is None
            return self.K_diag(X, variance)

        else:
            return self.K(variance, lengthscales, X, X2)

    def __add__(self, other: "BayesianKernel") -> "BayesianKernel":
        return Sum([self, other])

    def __mul__(self, other: "BayesianKernel") -> "BayesianKernel":
        return Product([self, other])


class BayesianCombination(BayesianKernel):
    """
    Combine a list of bayesian kernels, e.g. by adding or multiplying (see inheriting
    classes).
    ...
    def __init__(self, kernels: Sequence[BayesianKernel], name: Optional[str] = None) -> None:
        super().__init__(name=name)

        if not all(isinstance(k, BayesianKernel) for k in kernels):
            raise TypeError("can only combine Bayesian Kernel instances")  # pragma: no cover

        self.kernels: List[BayesianKernel] = []
        self._set_kernels(kernels)

    def _set_kernels(self, kernels: Sequence[BayesianKernel]) -> None:
        # add kernels to a list, flattening out instances of this class therein
        kernels_list: List[BayesianKernel] = []
        for k in kernels:
...
class ReducingBayesianCombination(BayesianCombination):
    def __call__(
        self,
        variance: TensorType,
        X: TensorType,
        ...
        *,
        ...
        return self._reduce(
            [k(variance, X, X2, lengthscales, full_cov=full_cov, presliced=presliced) for k in self.kernels]
        )
    ...
    def K(self,
        ...
        X: TensorType, 
        X2: Optional[TensorType] = None,
        lengthscales: Optional[TensorType] = None) -> tf.Tensor:
        return self._reduce([k.K(variance, X, X2, lengthscales) for k in self.kernels])

    def K_diag(self, X: TensorType,
        variance: TensorType) -> tf.Tensor:
        return self._reduce([k.K_diag(X, variance) for k in self.kernels])
...
class Sum(ReducingBayesianCombination):
    @property
    def _reduce(self) -> Callable[[Sequence[TensorType]], TensorType]:
        return tf.add_n  # type: ignore


class Product(ReducingBayesianCombination):
    @property
    def _reduce(self) -> Callable[[Sequence[TensorType]], TensorType]:
